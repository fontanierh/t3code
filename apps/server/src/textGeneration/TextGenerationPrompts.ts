...
const INITIAL_THREAD_TITLE_PROMPT = `Generate a title that will help the user recognize this Crab thread weeks later.
...
function regenerateThreadTitlePrompt(previousTitle: string): string {
  return `Regenerate the title for an existing Crab thread so the user can recognize it weeks later.
