...
This guide is for people who want to use more than one Claude setup in Crab. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Crab](./install.md).
...
In Crab Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Crab uses Claude Code's normal config directory.

When you set this field, Crab points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, Crab
...
Crab looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Crab looks.

Then add another Claude provider in Crab:
...
Crab only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Crab keeps separate config directories isolated
...
Add or edit a Claude provider in Crab Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Crab stores the value as a server secret and does not
...
Crab does not need a special Claude Code Router provider. Treat the router as a Claude
