# Keeping Crab in Sync

The Crab web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Crab shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                               |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Update server**          | Available for the Crab Linux background service. Select the button and leave Crab open while it prepares, tests, restarts, and reconnects.                               |
| **Update the desktop app** | Open the Crab desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Crab server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Crab does not update connected
...
If the server instead runs as the Crab background service, update the service on the host and
...
See [Running Crab in the Background](./background-service.md) for install, status, and removal
commands.
