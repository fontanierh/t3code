...
This guide is for people who want to use more than one Codex account in Crab. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Crab](./install.md).
...
issue. Crab uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
and share with OpenAI employees.

## Approve access to other apps

When a Codex tool needs access to an app such as Safari, Crab shows the app name and asks for
...
In Crab Settings, name it something obvious:
...
In Crab Settings, add another Codex provider:
...
Crab shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Crab treats it
